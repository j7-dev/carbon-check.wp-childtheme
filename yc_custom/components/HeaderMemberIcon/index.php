	...
	if ('icon' === $widget->get_name()) {
		...
		if ('carbon_header_member_icon' === $settings['_element_id']) {
			...
			if (is_user_logged_in()) {
				...
				$html .= '<ul class="dropdown-items absolute bg-white rounded-xl p-4 shadow-lg w-40 right-0 lg:right-8 list-none" style="display:none"><li class="my-2"><a class="text-black" href="' . site_url('profile') . '">帳號設定<a></li><li class="my-2"><a class="text-black" href="' . wp_logout_url(home_url()) . '">登出<a></li></ul>';

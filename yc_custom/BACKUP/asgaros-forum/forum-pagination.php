...
class AsgarosForumPagination
{
  ...
  public function renderPagination($location, $sourceID = false, $element_counter = false)
  {
    ...
    if ($location == $this->asgarosforum->tables->posts) {
      ...
      // 全文搜尋
      $query_match_name = "SELECT id AS topic_id FROM {$this->asgarosforum->tables->topics} WHERE name LIKE '%{$this->asgarosforum->search->search_keywords_for_query}%'";
      $query_match_text = "SELECT parent_id AS topic_id FROM {$this->asgarosforum->tables->posts} WHERE text LIKE '%{$this->asgarosforum->search->search_keywords_for_query}%'";
      // END 全文搜尋
